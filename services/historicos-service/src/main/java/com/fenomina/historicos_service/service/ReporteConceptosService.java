    ...
    public Page<ReporteVacacionesEmpresaDTO> getVacacionesPorEmpresa(
        ...
        List<ReporteVacacionesEmpresaDTO> dtos = rawPage.getContent().stream()
                .map(row -> ReporteVacacionesEmpresaDTO.builder()
                        .documentoEmp((String) row[1])
                        .nombresEmp((String) row[2])
                        .apellidosEmp((String) row[3])
                        .fechaInicioVac(toLocalDate(row[4]))
                        .fechaFinVac(toLocalDate(row[5]))
                        .tipoVacaciones(row[6] != null ? row[6].toString() : null)
                        .diasTomados(toInteger(row[7]))
                        .estadoVacaciones(row[8] != null ? row[8].toString() : null)
                        .valorPagoVac(toBigDecimal(row[9]))
                        .build())
                .collect(Collectors.toList());
        ...
        List<ReporteVacacionesTotalDTO> resultado = porPeriodo.entrySet().stream()
                .map(entry -> {
                    ...
                    return ReporteVacacionesTotalDTO.builder()
                            ...
                            .periodo(Integer.parseInt(parts[1]))
                            .fechaPeriodo(null)
                            .totalVacacionesRemuneradas(disfrutadas)
                            .totalVacacionesNoRemuneradas(compensadas)
                            .totalVacaciones(disfrutadas.add(compensadas))
                            .build();
                })
